...
struct NodesView : public Application {
    ...
    void OnStop() override { ed::DestroyEditor(m_Context); }
    ...
    void OnFrame(float deltaTime) override {
        ...
        if (do_popup) {
            ...
            do_popup = false;  // disable bool so that if we click off the
                               // popup, it doesn't open the next frame.
        }
        ...
        if (ImGui::BeginPopup("popup_button")) {
            ...
            if (ImGui::Button("Option 1")) {
                ...
                ImGui::CloseCurrentPopup();  // These calls revoke the popup
                                             // open state, which was set by
                                             // OpenPopup above.
            }
